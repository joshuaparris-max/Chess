...
export default function FamilyPuzzles() {
  ...
  const puzzle = FAMILY_PUZZLES[idx];
  ...
  const loadPuzzle = (newIdx: number) => {
    setIdx(newIdx);
    setGame(new Chess(FAMILY_PUZZLES[newIdx].fen));
    setSelected(null);
    setAttempts(0);
    setHintUsed(false);
    setSolved(false);
  ...
  const totalStars = FAMILY_PUZZLES.reduce((sum, p) => sum + (progress.puzzleStars[p.id] ?? 0), 0);
  const maxPossible = FAMILY_PUZZLES.reduce((sum, p) => sum + p.maxStars, 0);
    ...
    return (
      <div>
        <CelebrationOverlay
          ...
          boardPrompt={idx < FAMILY_PUZZLES.length - 1 ? undefined : "You finished all the puzzles! Great work."}
